...
param([string]$ConfigPath = "$PSScriptRoot\SageUnity.Local.config")
...
function New-ConfigSettings([xml]$config)
{
    ...
    elseif ($config.InstallSettings.Database.DatabaseInstallPath.DataFiles.UseAppFolder)
    {
        # Use application folder
        $local = $config.InstallSettings.WebServer.SitecoreInstallRoot + "\" + $config.InstallSettings.WebServer.SitecoreInstallFolder + "\" + $config.InstallSettings.Database.DatabaseInstallPath.DataFiles.LocalFolder
    }
    else 
    {
        # Use SQL Server data folder
        $sqlServerName = $config.InstallSettings.Database.SqlServerName
        [System.Reflection.Assembly]::LoadWithPartialName('Microsoft.SqlServer.SMO') | out-null
        $sqlConfig = New-Object ('Microsoft.SqlServer.Management.Smo.Server') "$sqlServerName"
        $local = $sqlConfig.DefaultFile + $config.InstallSettings.Database.DatabaseInstallPath.DataFiles.LocalFolder
    }
...
function Test-PreRequisites
{
    ...
    {
        # Changed - check ImportExcel module only if needed to use 
        $moduleName = "ImportExcel"
        if (!(Test-Module $moduleName) -and !([string]::IsNullOrEmpty($script:configSettings.SitecoreConfigSpreadsheetPath)))
        {
...
function Test-ScriptPermissionForPath([string]$path)
{
    ...
    $split = $path.Split('\')
    $computer = if ($uri.Host -eq "") { $env:COMPUTERNAME } else { $uri.Host }
    ...
    try
    {
        ...
        $sharedSecurity = Get-WMIObject -Class Win32_LogicalShareSecuritySetting   -ComputerName $computer #-Filter "name='$share'"
...
function Test-SqlInstallPaths
{
    ...
    [Microsoft.SqlServer.Management.Smo.Server]$sqlServerSmo = Get-SqlServerSmo        
    $sqlServiceUser = $sqlServerSmo.SqlDomainGroup
    ...
    foreach ($path in $installPaths)
    {
        ...
        # Set permissions
        $Acl = Get-Acl $path
        $Ar = New-Object System.Security.AccessControl.FileSystemAccessRule($sqlServiceUser,"FullControl", "ContainerInherit,ObjectInherit", "None", "Allow")
...
function Test-ConfigurationSettings
{
    ...
    if ($versionToInstall -ge 10.0)
    {
        ...
        {
            # Changed - SitecoreConfigSpreadsheetPath can be empty for local dev environment purposes
    ...
    if ($script:configSettings.Database.Enabled)
    {
        ...
        {
            if ($script:configSettings.Database.DatabaseInstallPath.DataFiles.UseAppFolder -and !(Test-SqlInstallPaths))
            {
            ...
            if (!([string]::IsNullOrEmpty($script:configSettings.Database.DatabaseInstallPath.DataFiles.Local)) -and !(Test-SqlInstallPaths))
            {
                Write-Message "DatabaseInstallPath is not valid." "Red" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
                return $FALSE
            }
...
function Initialize-SitecoreInstaller([string]$configPath)
{
    ...
    $versionToInstall = Get-SitecoreVersion -GetFromZip
    if ($versionToInstall -eq "10.0" -and $script:configSettings.SitecoreConfigSpreadsheetPath -ne "")
    {
...
function Install-SitecoreApplication([string]$configPath, [bool]$SuppressOutputToScreen=$FALSE)
{
    ...
    try
    {
        Write-Message "Sage Unity installer" "Green" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
