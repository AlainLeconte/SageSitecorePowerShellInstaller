...
function Set-ConfigurationFiles
{
    ...
    if ($sitecoreVersion -eq 8.0)
    {
        ...
        $webconfig.configuration.SelectSingleNode("sitecore/settings/setting[@name='LicenseFile']").SetAttribute("value", "`$(dataFolder)/license/license.xml")
    }
...
function Initialize-SitecoreInstaller([string]$configPath)
{
	...
	New-ConfigSettingsForModules $configXml
	# << Modules Installation
